...
func (svd SigVerificationDecorator) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (newCtx sdk.Context, err error) {
	...
	for i, sig := range sigs {
		...
		if sig.Sequence != currSequence {
			return ctx, sdkerrors.Wrapf(
				sdkerrors.ErrWrongSequence,
				"account sequence mismatch, expected %d, got %d", currSequence, sig.Sequence,
			)
		}
		...
		if !simulate && !ctx.IsReCheckTx() {
			...
			if err != nil {
				...
				if OnlyLegacyAminoSigners(sig.Data) {
					...
					errMsg = fmt.Sprintf("signature verification failed; please verify account number (%d), sequence (%d) and chain-id (%s)", accNum, currSequence, chainID)
				} else {
